...
def main():
    ...
    print_command(
        "uv sync", "Install all project dependencies", "Only needed first time or after pyproject.toml changes"
    )
    print_command("python verify_setup.py", "Verify the setup is correct", "Checks all files, config, and dependencies")
    print_command("python test_imports.py", "Test if all Python imports work", "Useful for debugging dependency issues")
    ...
    print_command(
        "python run_dashboard.py", "Alternative launcher script", "Does the same as the streamlit command above"
    )
    print_command(
    ...
    print_command(
        "python setup_dashboard.py", "Interactive setup wizard", "Guides you through installation step-by-step"
    )
    print_command("streamlit cache clear", "Clear Streamlit's internal cache", "Try this if having stale data issues")
    print_command("ls reports/figures/", "List generated prediction grids", "Check what grids are available locally")
    ...
    print_command("python --version", "Check Python version", "Should be 3.11 or higher")
    print_command("uv pip list", "List all installed packages with versions", "Verify all dependencies are present")
    print_command("pip install --upgrade streamlit", "Upgrade Streamlit to latest version", "Use if experiencing bugs")
...
"""
    ...
    issues = [
        ...
        ("ImportError: No module named 'streamlit'", "uv sync"),
        ("No prediction grids found", "Click 'Generate New Samples' in dashboard"),
        ("Model not found error", "Check configs/config.yaml for correct model path"),
        ("Generation is slow", "Normal: YOLOv8n takes 2-10 min for 50 test images"),
    ]
